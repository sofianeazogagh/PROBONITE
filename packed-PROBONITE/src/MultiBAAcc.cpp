...
Ciphertext<DCRTPoly> MultiMerge(Ciphertext<DCRTPoly> ct, int expansion_factor, int number_of_clients, CryptoContext<DCRTPoly> cc) //expansion factor = max slots
{
  ...
  for (int i = 1; i < number_of_clients ; i++)
  {
    ...
    auto ct3 = cc->EvalMerge(Ciphertexts);
...
Ciphertext<DCRTPoly> MultiOHS(Ciphertext<DCRTPoly> ct_I, int max_slots, int number_of_clients, CryptoContext<DCRTPoly> cc, Plaintext ptT, Plaintext ptoneN)
{
  ...
  // only a-th slot is 0
  auto cta = MultiMerge(ct_I, max_slots,number_of_clients,cc);
...
void UniTESTMultiOHS(CryptoContext<DCRTPoly> cc, KeyPair<DCRTPoly> keyPair, int max_slots, int number_of_clients, vector<int64_t> clear_I)
{
  ...
  ofstream fichier("../perf.txt", ios::out | ios::app);
...
//   SecurityLevel securityLevel = HEStd_128_classic;
  ...
  // timeval t_start_ohs, t_end_ohs;
  // gettimeofday(&t_start_ohs, NULL);
  // auto ohs = MultiOHS(ct_I, max_slots,number_of_clients,cc);
  // gettimeofday(&t_end_ohs, NULL);
  // cout << "Time Multi OHS : " << std::setprecision(5) << getMillies(t_start_ohs, t_end_ohs) << " ms" << '\n';
  // auto ct_baacc = cc->EvalMult(ct_concat,ohs);

  // vector<Ciphertext<DCRTPoly>> Features_selected = DeconcatCiphertexts(ct_baacc,number_of_clients,max_slots,cc);
