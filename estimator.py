...
- ``alpha``: where Î± a positive real number, (discrete) Gaussian distribution with parameter Î±
- ``"uniform"`` : uniform mod q (alias: ``False``)
- ``(a,b)`` : uniform in the interval ``[a,â€¦,b]``
- ``((a,b), h)`` : exactly ``h`` components are ``âˆˆ [a,â€¦,b]âˆ–\\{0\\}``, all other components are zero
...
class Param:
    ...
    def tuple(lwe):
        """
        Return (n, Î±, q) given an LWE instance object.
    ...
    def dict(lwe):
        """
        Return dictionary consisting of n, Î±, q and samples given an LWE instance object.

        :param lwe: LWE object
        :returns: "n": n, "alpha": Î±, "q": q, "samples": samples
        :rtype: dictionary
    ...
    def preprocess(n, alpha, q, success_probability=None, prec=None, m=oo):
        """
        Check if parameters n, Î±, q are sound and return correct types.
...
def stddevf(sigma):
    """
    Gaussian width parameter Ïƒ â†’ standard deviation

    :param sigma: Gaussian width parameter Ïƒ
...
def sigmaf(stddev):
    """
    standard deviation â†’ Gaussian width parameter Ïƒ
...
def alphaf(sigma, q, sigma_is_stddev=False):
    """
    Gaussian width Ïƒ, modulus q â†’ noise rate Î±
    ...
    :returns: Î± = Ïƒ/q or ÏƒÂ·sqrt(2Ï€)/q depending on `sigma_is_stddev`
...
class Cost:
    ...
    def str(self, keyword_width=None, newline=None, round_bound=2048, compact=False, unicode=True):
        ...
        """
        if unicode:
            unicode_replacements = {"delta_0": u"Î´_0", "beta": u"Î²", "epsilon": u"Îµ"}
        else:
        ...
        for k in d:
            ...
            if not newline and k in format_strings:
                ...
                t = u"%s" % (u"â‰ˆ" if unicode else "") + u"%s2^%.1f" % ("-" if v < 0 else "", log(abs(v), 2).n())
                if compact:
                    s.append(u"%s: %s" % (kk, t))
                else:
    ...
    def repeat(self, times, select=None, lll=None):
        ...
        For example, Î´ would not be multiplied but "#bop" would be. This check is strict such that
...
def switch_modulus(f, n, alpha, q, secret_distribution, *args, **kwds):
    ...
    :param f: run f
    :param n: LWE dimension `n > 0`
    :param alpha: noise rate `0 â‰¤ Î± < 1`, noise will have standard deviation `Î±q/\\sqrt{2Ï€}`
    :param q: modulus `0 < q`
    :param secret_distribution: distribution of secret, see module level documentation for details
...
class BKZ:
    ...
    def _delta_0f(k):
        """
        Compute `Î´_0` from block size `k` without enforcing `k` in ZZ.

        Î´_0 for kâ‰¤40 were computed as follows:
    ...
    !!            while i < trials:
                    ...
                    i += threads
                    print u"Î²: %2d, Î´_0: %.5f, time: %5.1fs, (%2d,%2d)"%(beta, mean(delta_0), mean(t), i, threads)
                print
        ```
    ...
    def _beta_secant(delta):
        """
        Estimate required blocksize `k` for a given root-hermite factor Î´ based on [PhD:Chen13]_
        ...
        .. [PhD:Chen13] Yuanmi Chen. RÃ©duction de rÃ©seau et sÃ©curitÃ© concrÃ¨te du chiffrement
                        complÃ¨tement homomorphe. PhD thesis, Paris 7, 2013.
        """
    ...
    def svp_repeat(beta, d):
        """Return number of SVP calls in BKZ-Î²
    ...
    def GSA(n, q, delta, m):
        ...
        .. [RSA:LinPei11] Richard Lindner and Chris Peikert. Better key sizes (and attacks) for LWE-based encryption.
                          In Aggelos Kiayias, editor, CT-RSA 2011, volume 6558 of LNCS, pages 319â€“339. Springer,
                          February 2011.
        """
    ...
    def LLL(d, B=None):
        ...
        ..  [CheNgu11] Chen, Y., & Nguyen, P.  Q.  (2011).  BKZ 2.0: better lattice security
            estimates.  In D.  H.  Lee, & X.  Wang, ASIACRYPT~2011 (pp.  1â€“20).  : Springer,
            Heidelberg.
        """
        if B:
            return d ** 3 * B ** 2
    ...
    def LinPei11(beta, d, B=None):
        ...
        ..  [LinPei11] Lindner, R., & Peikert, C.  (2011).  Better key sizes (and attacks) for LWE-based
            encryption.  In A.  Kiayias, CT-RSA~2011 (pp.  319â€“339).  : Springer, Heidelberg.
    ...
    def _BDGL16_small(beta, d, B=None):
        u"""
         Runtime estimation given `Î²` and assuming sieving is used to realise the SVP oracle for small dimensions.
    ...
    def _BDGL16_asymptotic(beta, d, B=None):
        u"""
         Runtime estimation given `Î²` and assuming sieving is used to realise the SVP oracle.
    ...
    def CheNgu12(beta, d, B=None):
        u"""
         Runtime estimation given `Î²` and assuming [CheNgu12]_ estimates are correct.
    ...
    def ABFKSW20(beta, d, B=None):
        ...
        StehlÃ© and Weiqiang Wen.  Faster Enumeration-based Lattice Reduction: Root Hermite
        Factor $k^{1/(2k)}$ in Time $k^{k/8 + o(k)}$.  CRYPTO 2020.
        """
        if 1.5 * beta >= d or beta <= 92:  # 1.5Î² is a bit arbitrary, Î²â‰¤92 is the crossover point
    ...
    def ADPS16(beta, d, B=None, mode="classical"):
        u"""
         ...
         ..  [ADPS16] Edem Alkim, LÃ©o Ducas, Thomas PÃ¶ppelmann, & Peter Schwabe (2016).
             Post-quantum key exchange - A New Hope.  In T. Holz, & S. Savage, 25th USENIX
             Security Symposium, USENIX Security 16 (pp. 327â€“343). USENIX Association.
         """
...
def delta_0f(beta):
    """
    Compute root-Hermite factor `Î´_0` from block size `Î²`.
    """
    beta = ZZ(round(beta))
...
def betaf(delta):
    """
    Compute block size `Î²` from root-Hermite factor `Î´_0`.
    """
...
def lattice_reduction_cost(cost_model, delta_0, d, B=None):
    """
    Return cost dictionary for returning vector of norm` Î´_0^d Vol(Î›)^{1/d}` using provided lattice
    reduction algorithm.

    :param lattice_reduction_estimate:
    :param delta_0: root-Hermite factor `Î´_0 > 1`
    :param d: lattice dimension
    :param B: bit-size of entries
...
def lattice_reduction_opt_m(n, q, delta):
    ...
    :param n: LWE dimension `n > 0`
    :param q: modulus `0 < q`
    :param delta: root Hermite factor `Î´_0`
...
def _primal_scale_factor(secret_distribution, alpha=None, q=None, n=None):
    ...
    :param secret_distribution: distribution of secret, see module level documentation for details
    :param alpha: noise rate `0 â‰¤ Î± < 1`, noise has standard deviation `Î±q/\\sqrt{2Ï€}`
    :param q: modulus `0 < q`
    :param n: only used for sparse secrets
    ...
    ..  [BaiGal14] Bai, S., & Galbraith, S.  D.  (2014).  Lattice decoding attacks on binary
        LWE.  In W.  Susilo, & Y.  Mu, ACISP 14 (pp.  322â€“337).  : Springer, Heidelberg.
    """
...
def _primal_usvp(
    ...
    # we have m samples â†’ the largest permissible dimension d is m+1
    if d is None:
...
def primal_usvp(
    ...
    **kwds
    ...
    ..  [USENIX:ADPS16] Alkim, E., LÃ©o Ducas, Thomas PÃ¶ppelmann, & Schwabe, P.  (2015).
...
def _primal_decode(
    ...
    ..  [LinPei11] Lindner, R., & Peikert, C.  (2011).  Better key sizes (and attacks) for
        LWE-based encryption.  In A.  Kiayias, CT-RSA~2011 (pp.  319â€“339).  : Springer, Heidelberg.
    """
...
def _dual_scale_factor(secret_distribution, alpha=None, q=None, n=None, c=None):
    ...
    if SDis.is_small(secret_distribution, alpha=alpha):
        ...
        if c is None:
            # |<v,s>| = |<w,e>| â†’ c * \sqrt{n} * \sqrt{Ïƒ_{s_i}^2 + E(s_i)^2} == \sqrt{m} * Ïƒ
...
def _dual(
    ...
    ..  [MicReg09] Micciancio, D., & Regev, O.  (2009).  Lattice-based cryptography.  In D.  J.
        Bernstein, J.  Buchmann, & E.  Dahmen (Eds.), Post-Quantum Cryptography (pp.  147â€“191).
...
def dual_scale(
    ...
    """
    Estimate cost of solving LWE by finding small `(y,x/c)` such that `y â‹… A â‰¡ c â‹… x \bmod q` as
...
def mitm(n, alpha, q, secret_distribution=True, m=oo, success_probability=0.99):
    ...
    .. [AlbPlaSco15] Albrecht, M. R., Player, R., & Scott, S. (2015). On the concrete hardness of
       Learning with Errors. Journal of Mathematical Cryptology, 9(3), 169â€“203.
...
def _bkw_coded(n, alpha, q, secret_distribution=True, m=oo, success_probability=0.99, t2=0, b=2, ntest=None):  # noqa
    ...
    :param alpha: noise rate `0 â‰¤ Î± < 1`, noise will have standard deviation `Î±q/\\sqrt{2Ï€}`
    :param q: modulus `0 < q`
    :param t2:                   number of coded BKW steps (â‰¥ 0)
    :param b:                    table size (â‰¥ 1)
    :param success_probability: targeted success probability < 1
    :param ntest:                optional parameter ntest

    """
    n, alpha, q, success_probability = Param.preprocess(n, alpha, q, success_probability)
    sigma = stddevf(alpha * q)  # [C:GuoJohSta15] use Ïƒ = standard deviation
    RR = alpha.parent()
    ...
    # we compute t1 from N_i by observing that any N_i â‰¤ b gives no advantage
    # over vanilla BKW, but the estimates for coded BKW always assume
    # quantisation noise, which is too pessimistic for N_i â‰¤ b.
    t1 = 0
    if ntest is None:
    ...
    # if there's no ntest then there's no `Ïƒ_{set}` and hence no ncod
    if ntest:
        sigma_set = sqrt(q ** (2 * (1 - l / ntest)) / 12)
        # cost[u"Ïƒ_set"] = RR(sigma_set)
        ncod = sum([N(i, sigma_set) for i in range(1, t2 + 1)])
    else:
        ncod = 0
    ...
    # cost[u"Ïƒ_final"] = RR(sigma_final)
...
def bkw_coded(n, alpha, q, secret_distribution=True, m=oo, success_probability=0.99, ntest=None):
    ...
    .. [GuoJohSta15] Guo, Q., Johansson, T., & Stankovski, P.  (2015).  Coded-BKW: solving LWE using lattice
       codes.  In R.  Gennaro, & M.  J.  B.  Robshaw, CRYPTO~2015, Part~I (pp.  23â€“42).  :
       Springer, Heidelberg.
...
def gb_cost(n, D, omega=2, prec=None):
    """
    Estimate the complexity of computing a GrÃ¶bner basis.

    :param n: number of variables `n > 0`
    :param D: tuple of `(d,m)` pairs where `m` is number polynomials and `d` is a degree
    :param omega: linear algebra exponent, i.e. matrix-multiplication costs `O(n^Ï‰)` operations.
...
def arora_gb(n, alpha, q, secret_distribution=True, m=oo, success_probability=0.99, omega=2):
    ...
    ..  [ACFP14] Albrecht, M.  R., Cid, C., Jean-Charles FaugÃ¨re, & Perret, L.  (2014).
        Algebraic algorithms for LWE.

    ..  [AroGe11] Arora, S., & Ge, R.  (2011).  New algorithms for learning in presence of
        errors.  In L.  Aceto, M.  Henzinger, & J.  Sgall, ICALP 2011, Part~I (pp.  403â€“415).  :
        Springer, Heidelberg.
    """
...
def estimate_lwe(  # noqa
    ...
    EXAMPLE::
        ...
        usvp: rop:  â‰ˆ2^57.3,  red:  â‰ˆ2^57.3,  Î´_0: 1.009214,  Î²:  101,  d:  349,  m:      220
         dec: rop:  â‰ˆ2^61.9,  m:      229,  red:  â‰ˆ2^61.9,  Î´_0: 1.009595,  Î²:   93,  d:  357,  ...
        dual: rop:  â‰ˆ2^81.1,  m:      380,  red:  â‰ˆ2^81.1,  Î´_0: 1.008631,  Î²:  115,  d:  380,  ...

        sage: d = estimate_lwe(**Param.LindnerPeikert(256, dict=True))
        usvp: rop: â‰ˆ2^127.8,  red: â‰ˆ2^127.8,  Î´_0: 1.005788,  Î²:  228,  d:  588,  m:      331
         dec: rop: â‰ˆ2^137.3,  m:      337,  red: â‰ˆ2^137.3,  Î´_0: 1.005976,  Î²:  217,  d:  593,  ...
        dual: rop: â‰ˆ2^166.0,  m:      368,  red: â‰ˆ2^166.0,  Î´_0: 1.005479,  Î²:  249,  repeat: ...
        ...
        usvp: rop:  â‰ˆ2^98.0,  red:  â‰ˆ2^98.0,  Î´_0: 1.006744,  Î²:  178,  d:  499,  m:      242,  ...
         dec: rop: â‰ˆ2^137.3,  m:      337,  red: â‰ˆ2^137.3,  Î´_0: 1.005976,  Î²:  217,  d:  593,  ...
        dual: rop: â‰ˆ2^108.4,  m:      270,  red: â‰ˆ2^108.3,  Î´_0: 1.006390,  Î²:  195,  repeat:  ...

        sage: d = estimate_lwe(*Param.LindnerPeikert(256), secret_distribution=(-1,1), reduction_cost_model=BKZ.sieve)
        Warning: the LWE secret is assumed to have Hamming weight 171.
        usvp: rop:  â‰ˆ2^80.3,  red:  â‰ˆ2^80.3,  Î´_0: 1.006744,  Î²:  178,  d:  499,  m:      242,  ...
         dec: rop: â‰ˆ2^111.8,  m:      369,  red: â‰ˆ2^111.8,  Î´_0: 1.005423,  Î²:  253,  d:  625,  ...
        dual: rop:  â‰ˆ2^90.6,  m:      284,  red:  â‰ˆ2^90.6,  Î´_0: 1.006065,  Î²:  212,  ...

        sage: d = estimate_lwe(n=100, alpha=8/2^20, q=2^20, skip="arora-gb")
        mitm: rop: â‰ˆ2^329.2,  m:       23,  mem: â‰ˆ2^321.5
        usvp: rop:  â‰ˆ2^32.4,  red:  â‰ˆ2^32.4,  Î´_0: 1.013310,  Î²:   40,  d:  141,  m:       40
         dec: rop:  â‰ˆ2^34.0,  m:      156,  red:  â‰ˆ2^34.0,  Î´_0: 1.021398,  Î²:   40,  d:  256,  ...
        dual: rop:  â‰ˆ2^35.5,  m:      311,  red:  â‰ˆ2^35.5,  Î´_0: 1.014423,  Î²:   40,  d:  311,  ...
         bkw: rop:  â‰ˆ2^53.6,  m:  â‰ˆ2^43.5,  mem:  â‰ˆ2^44.5,  b:   2,  t1:   5,  t2:  18,  l:   1,  ...

        sage: d = estimate_lwe(n=100, secret_distribution=alphaf(1, 2^20, True), alpha=8/2^20, q=2^20)
        usvp: rop:  â‰ˆ2^32.2,  red:  â‰ˆ2^32.2,  Î´_0: 1.013310,  Î²:   40,  d:  127,  m:       26
         dec: rop:  â‰ˆ2^34.0,  m:      156,  red:  â‰ˆ2^34.0,  Î´_0: 1.021398,  Î²:   40,  d:  ...
        dual: rop:  â‰ˆ2^35.5,  m:      311,  red:  â‰ˆ2^35.5,  Î´_0: 1.014423,  Î²:   40,  d:  ...
